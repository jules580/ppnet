...
}' /opt/gatling/results/$1/simulation.log >> result.txt
...
test=$(grep -i WARN /opt/gatling/results/$1/gatling.txt)

counter=$(grep -o WARN /opt/gatling/results/$1/gatling.txt| wc -l)
grep -i 'request count' /opt/gatling/results/$1/gatling.txt > test8.txt
awk -F " " '{print "\""$2"_"$3"\""":"$4","}' test8.txt>> result.txt

echo '"time":[' >>result.txt
grep -i ' t ' /opt/gatling/results/$1/gatling.txt > test12.txt
awk -F " " '{if($2!="800")
...
echo "${z%?}" > result.txt
echo "]," >> result.txt
grep -i time /opt/gatling/results/$1/gatling.txt > test9.txt
...
grep -i 'Global:' /opt/gatling/results/$1/gatling.txt| sed '/^ *Global/s/://' > test13.txt
awk -F " " '{ print "{\"type\":\""$1"\",\"stat\":\""$2"_"$3"_"$4"_"$5"_"$6"_"$7"_"$8"\",\"param\":"$9",\"status\":\""$11"\"},"}' test13.txt >> result.txt
grep -i 'For' /opt/gatling/results/$1/gatling.txt | sed '1d' | sed '/^ *For all requests/s/://' > test14.txt
