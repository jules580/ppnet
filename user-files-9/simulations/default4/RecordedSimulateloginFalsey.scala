...
class RecordedSimulateloginFalsey extends Simulation {
before{
    ...
    val uri1 = "http://coucou/_matrix/client"
...
val admins=scenario("Admins").exec(Index.index,Login.login)
	setUp(admins.inject(
	rampUsers(10101010) over ( 11111111 seconds))
	.throttle(reachRps(10) in (10 seconds), holdFor(15 seconds),jumpToRps(50),holdFor(10 seconds)))
	.assertions(
	global.responseTime.max.lessThan(10111011),
	forAll.failedRequests.count.lessThan(10111110),
