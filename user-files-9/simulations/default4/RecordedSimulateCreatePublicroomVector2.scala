...
class RecordedSimulateCreatePublicroomVector2 extends Simulation {
...
before{
	...
	object Login{
		...
		.exec(http("Login page")
			...
			.check(status.is(200)),
            http("Sync filter")
			.get("/_matrix/client/r0/sync?filter=0&timeout=30000&_cacheBuster=1468424089625&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("minimise image")
			...
			.check(status.is(304)),
            http("Sync filter 2")
			.get("/_matrix/client/r0/sync?filter=0&timeout=30000&since=s1509_1233_764_7_1_1&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("cancel image")
	...
	object CreatePublicRoom{
		val createpublicroom=exec(http("create room")
			...
			.check(status.is(200)),
            http("Infos event")
			.get("/_matrix/client/r0/sync?filter=0&timeout=30000&since=s1511_1233_764_7_1_1&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("Voice image")
			...
			.check(status.is(304)),
          /*  http("Info user")
			.get("/_matrix/client/r0/rooms/${table}%3Amatrix.allmende.io/messages?from=s1510_1233_764_7_1_1&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),*/
            http("edit image")
			...
			.check(status.is(304)),
            /*http(" Time Room")
			.get("/_matrix/client/r0/rooms/${table}%3Amatrix.allmende.io/messages?from=t1-1509_1233_764_7_1_1&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),*/
            http("admin image")
	...
	val admin=scenario("Admins").exec(Init.init,Login.login,CreatePublicRoom.createpublicroom)
