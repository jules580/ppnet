...
class RecordedSimulateCreatePrivateRoomVector2 extends Simulation {
...
before{
	...
	object Init{
		...
		.exec(http("Login Page")
			...
			.check(status.is(200)),
            http("Presence Users")
			.options("/_matrix/client/r0/presence/%40test01%3Amatrix.allmende.io/status?access_token=${token}")
			.headers(headers_13)
			.check(status.is(200)),
            http("Turn Server")
			...
			.check(status.is(200)),
            http("")
			.get("/_matrix/client/r0/sync?filter=2&timeout=30000&_cacheBuster=1468424478508&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("Cancel Image")
			...
			.check(status.is(304)),
            http("sync filter")
			.get("/_matrix/client/r0/sync?filter=2&timeout=30000&since=s1532_1236_764_7_1_2&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("plus image")
	...
	object Parameter{
		val parameter=exec(http("request_53")
			.get("/_matrix/client/r0/sync?filter=2&timeout=30000&since=s1538_1237_764_7_1_2&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)))
		.pause(6)
	}
