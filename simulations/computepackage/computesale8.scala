...
class computesale8 extends Simulation {
  ...
  exec(http("Home")
   ...
   val stdUser = scenario("Standard User").exec(Search.search, Browse.browse)
   val admUser = scenario("Admin User").exec(Search.search, Browse.browse, Edit.edit)
   val advUser = scenario("Advanced User").exec(Search.search, Browse.browse)	
	
	
	setUp(
	users.inject(rampUsers(10) over (10 seconds)),
	admins.inject(rampUsers(2) over (10 seconds)),
	stdUser.inject(atOnceUsers(20)),
	admUser.inject(nothingFor(60 seconds), rampUsers(5) over (12 seconds)),
	advUser.inject(rampUsers(5) over (10 seconds))
