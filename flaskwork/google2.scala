...
val feeder=jsonUrl("http://0.0.0.0:7676/test.json").random
...
before{
	...
	object Index{
		val feeders=jsonUrl("http://0.0.0.0:7676/test.json").random
		...
		object Login{
			 val feeder =csv("login.csv").random
			val feeders=jsonUrl("http://0.0.0.0:7676/test.json")
