...
import Trait._
	...
	val createpublicroom2=exec(http("create room")
			...
			.check(status.is(304)),
          http("Info user")
			.get("/_matrix/client/r0/rooms/${room_id}%3Amatrix.allmende.io/messages?from=s1510_1233_764_7_1_1&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("edit image")
			...
			.check(status.is(304)),
            http(" Time Room")
			.get("/_matrix/client/r0/rooms/${room_id}%3Amatrix.allmende.io/messages?from=t1-1509_1233_764_7_1_1&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            http("admin image")
