...
let "DernierArgument=DernierArgument-1"
...
infos2="/home/tai/Bureau/gatling-charts-highcharts-bundle-2.2.2/userfiles/simulations/test/" $3
echo $infos2 > gatling35.txt
infos5=$(cut -d'.' -f1 gatling35.txt)
echo $infos2 > gatling34.txt
infos4=$(cut -d'y' -f1 gatling34.txt)

sed -re 's/coucou/'$(echo $URL)'/g' $
...
grep -i 'Global:' gatling.txt> test13.txt
cat test13.txt
grep -i 'For ' gatling.txt > test14.txt
cat test14.txt
 rm -f test9.txt
 rm -f test10.txt
 rm -f test11.txt
