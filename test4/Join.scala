...
object Join2 extends Trait{
	val join2=exec(http("Join Possibility")
            ...
            http("")
			.get(url+"/_matrix/client/r0/sync?filter=0&timeout=30000&since=s1542_1239_764_7_1_2&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
            /*http("call image")
			...
			.check(status.is(304)),*/
            http("Infos room")
			.get(url+"/_matrix/client/r0/rooms/${table}%3Amatrix.allmende.io/messages?from=s1541_1239_764_7_1_2&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)),
          /*  http("f4c371 image")
			...
			.check(status.is(200)),
            http("time enter")
			.get(url+"/_matrix/client/r0/rooms/${table}%3Amatrix.allmende.io/messages?from=t1-1540_1239_764_7_1_2&limit=20&dir=b&access_token=${token}")
			.headers(headers_14)
			.check(status.is(200)))
            /*http("admin image")
