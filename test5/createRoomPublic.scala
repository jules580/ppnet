...
val createroom_matrix2=exec(http("Create Room")
			...
			.resources(
			http("Room connected")
			.get("/_matrix/client/api/v1/rooms/${Room_Id}%3Amatrix.allmende.io/initialSync?access_token=${token}&limit=30")
			.headers(headers_4)
			.check(status.is(200)),
            http("Event1")
           ...
           http("Back to the room")
			.get("/_matrix/client/api/v1/rooms/${Room_Id}%3Amatrix.allmende.io/messages?access_token=${token}&from=t1-79_218_18_1_1_1&limit=30")
			.headers(headers_4)
			.check(status.is(200)))
			.check(status.is(200)))
			.exec(session =>{
