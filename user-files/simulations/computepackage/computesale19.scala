...
class computesale19 extends Simulation {
before{
...
val chain2 = exec(session => {
 ...
 exec(http("Home")
    ...
    .feed(feeder)
    // 3
    .exec(http("Search ${searchComputerName}")
    .get("/computers?f=${searchCriterion}") 
    .check(css("a:contains('${searchComputerName}')", "href").saveAs("computerURL"))

    .check(status.find.in(200,202).saveAs("status"))
    .check(regex("Computer database").count.greaterThanOrEqual(1).saveAs("Infos")))
.exec(session =>{
	println("test:" +session)
	//println("test5':"+session("searchComputerName").as[String])	
	val foo = Success("Amstrad CPC")
...
.assertions(
	...
	details("Search 0").failedRequests.percent.is(0),
	details("Search 0").requestsPerSec.lessThan(10),
details("Search 0").requestsPerSec.between(100, 1000),
	forAll.failedRequests.count.greaterThan(50),
