...
test=$(grep -i WARN $1/gatling.txt)

counter=$(grep -o WARN $1/gatling.txt| wc -l)
grep -i 'request count' $1/gatling.txt > test8.txt
awk -F " " '{print "\""$2"_"$3"\""":"$4","}' test8.txt>> result.txt

echo '"time":[' >>result.txt
grep -i ' t ' $1/gatling.txt > test12.txt
awk -F " " '{if($2!="800")
...
echo "${z%?}" > result.txt
echo "]," >> result.txt
grep -i time $1/gatling.txt > test9.txt
