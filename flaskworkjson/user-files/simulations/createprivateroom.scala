...
object CreatePrivateRoom_Matrix2 extends Trait_Matrix{
	...
	val createprivateroom_matrix2= exec(feed(feeder))
		.exec(http("CreateRoom")
			...
			.get("${url}/_matrix/client/api/v1/events?access_token=${token}&from=s166_250_20_1_1_1&timeout=30000")
			.check(status.is(200)),
            http("Event form the room 2")
			.get("${url}/_matrix/client/api/v1/events?access_token=${token}&from=s167_251_20_1_1_1&timeout=30000")
			.check(status.is(200)),
            http("Event from the room3")
			.get("${url}/_matrix/client/api/v1/events?access_token=${token}&from=s168_251_20_1_1_1&timeout=30000")
			.check(status.is(200)),
            http("Event form the room4")
			.get("${url}/_matrix/client/api/v1/events?access_token=${token}&from=s169_251_20_1_1_1&timeout=30000")
			.check(status.is(200)),
          /*  http("access Room2")
			.get("/_matrix/client/api/v1/rooms/!hhYFDErHCDGoMIPUNU%3Amatrix.allmende.io/initialSync?access_token=${token}&limit=30")
			.check(status.is(200)),*/
            http("request_15")
			.get("${url}/_matrix/client/api/v1/events?access_token=${token}&from=s170_251_20_1_1_1&timeout=30000")
			.check(status.is(200)),
            http("Infos identity")
           ...
           // http("Infos room")
			...
			.doIfOrElse(_.contains("Room_Id")){
				exec(session=>{
					println("Sucess Create Private Room")
					session
				}
				)
			}{
				exec(session =>{
					println("Fail Create Private Room")
					session
				}
				)
			}
